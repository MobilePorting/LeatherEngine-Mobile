...
class ChartingStateRevamp extends MusicBeatState {
	// Constants
	...
	var Beat_Snap:Int = 16;
	var Beat_Snaps:Array<Int> = [4, 6, 8, 12, 16, 24, 32, 48, 64, 128, 192];
	var Dummy_Note:FlxSprite;
	...
	override function create() {
		...
		if (FlxG.mouse.x > Note_Grid.x
			&& FlxG.mouse.x < Note_Grid.x + Note_Grid.width
			&& FlxG.mouse.y > Note_Grid.y
			&& FlxG.mouse.y < Note_Grid.y + (Grid_Size * (Conductor.stepsPerSection))) {
			var snappedGridSize = (Grid_Size / (Beat_Snap / Conductor.stepsPerSection));

			Dummy_Note.x = Math.floor(FlxG.mouse.x / Grid_Size) * Grid_Size;

			if (FlxG.keys.pressed.SHIFT)
				Dummy_Note.y = FlxG.mouse.y;
			else
				Dummy_Note.y = Math.floor(FlxG.mouse.y / snappedGridSize) * snappedGridSize;
		}
	}

	inline function getStrumTime(yPos:Float):Float {
		return FlxMath.remapToRange(yPos, Note_Grid.y, Note_Grid.y + Note_Grid.height, 0, Conductor.stepsPerSection * Conductor.stepCrochet);
	}

	function addNote():Void {
		if (SONG.notes[Cur_Section] == null)
			addSection();

		var noteStrum = getStrumTime(Dummy_Note.y) + sectionStartTime();
		var noteData = Math.floor((FlxG.mouse.x / Grid_Size) - 1);
		var noteSus = 0;
		var characters:Array<Int> = [];

		if (Note_Type != "default" && Note_Type != null)
			SONG.notes[Cur_Section].sectionNotes.push([
				noteStrum,
				noteData,
				noteSus,
				(characters.length <= 0 ? Cur_Note_Char : characters),
				Note_Type
			]);
		else
			SONG.notes[Cur_Section].sectionNotes.push([
		...
		Selected_Note = SONG.notes[Cur_Section].sectionNotes[SONG.notes[Cur_Section].sectionNotes.length - 1];

		if (FlxG.keys.pressed.CONTROL) {
			if (Note_Type != "default")
				SONG.notes[Cur_Section].sectionNotes.push([
					noteStrum,
					(noteData + SONG.keyCount) % (SONG.keyCount + SONG.playerKeyCount),
					noteSus,
					Cur_Note_Char,
					Note_Type
				]);
	...
	override function update(elapsed:Float) {
		...
		if (controls.RIGHT_P && FlxG.keys.pressed.CONTROL) {
			if (Beat_Snaps.indexOf(Beat_Snap) + 1 <= Beat_Snaps.length - 1)
				Beat_Snap = Beat_Snaps[Beat_Snaps.indexOf(Beat_Snap) + 1];
		}

		if (controls.LEFT_P && FlxG.keys.pressed.CONTROL) {
			if (Beat_Snaps.indexOf(Beat_Snap) - 1 >= 0)
				Beat_Snap = Beat_Snaps[Beat_Snaps.indexOf(Beat_Snap) - 1];
		}
		...
		if (FlxG.mouse.justPressed) {
			if (FlxG.mouse.x > Note_Grid.x
				&& FlxG.mouse.x < Note_Grid.x + Note_Grid.width
				&& FlxG.mouse.y > Note_Grid.y
				&& FlxG.mouse.y < Note_Grid.y + Note_Grid.height) {
				addNote();
			}
		}
	...
	function updateGrid() {
		...
		Dummy_Note = new FlxSprite().makeGraphic(Grid_Size, Grid_Size);
		add(Dummy_Note);
